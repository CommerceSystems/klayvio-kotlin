/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package org.openapitools.client.models

import org.openapitools.client.models.GetProfileResponseCollectionDataInnerAllOfAttributes
import org.openapitools.client.models.GetProfileResponseCollectionDataInnerAllOfRelationships
import org.openapitools.client.models.ObjectLinks
import org.openapitools.client.models.ProfileEnum

import com.squareup.moshi.Json
import com.squareup.moshi.JsonClass

/**
 * 
 *
 * @param type 
 * @param attributes 
 * @param links 
 * @param id Primary key that uniquely identifies this profile. Generated by Klaviyo.
 * @param relationships 
 */


data class GetSegmentMemberResponseCollectionDataInner (

    @Json(name = "type")
    val type: ProfileEnum,

    @Json(name = "attributes")
    val attributes: GetProfileResponseCollectionDataInnerAllOfAttributes,

    @Json(name = "links")
    val links: ObjectLinks,

    /* Primary key that uniquely identifies this profile. Generated by Klaviyo. */
    @Json(name = "id")
    val id: kotlin.String? = null,

    @Json(name = "relationships")
    val relationships: GetProfileResponseCollectionDataInnerAllOfRelationships? = null

) {


}

